...
impl Scene {
    ...
    fn move_to_origin(&self, round: &[usize;2], axis: Axis) -> [usize;2] {
        ...
        let number_of_rounded_before = self
            ...
            .filter(|r| {
                r[f_axis]== f_col_row
                    && r[m_axis] < start
                    && if let Some(next_cubed_pos) = next_cubed_pos { r[m_axis] > next_cubed_pos } else {true}
            })
            .count();

        let mut result=*round;
        result[m_axis]= if let Some(next_cubed_pos) = next_cubed_pos { next_cubed_pos +1 } else {0}+number_of_rounded_before;
        result
    ...
    fn move_away_origin(&self, round: &[usize;2], axis: Axis) -> [usize;2] {
        ...
        }) - 1 - number_of_rounded_after;


        result
    }

    fn tilt(&self,round:&Vec<[usize;2]>,direction : Direction)->Vec<[usize;2]> {
        let mut result:Vec<[usize;2]> = round.iter().map(|r|match direction{
    ...
    fn tune(&mut self)    {
        ...
        previous.insert(self.rounded.iter().copied().collect(),0);
    ...
    fn print(&self) {
        ...
        for y in 0..self.maxy{
            ...
            // }
            println!("{}", (0..self.maxx).map(|x|if self.cubed.contains(&[x,y]){'#'}else if self.rounded.contains(&[x,y]){'O'}else{'.'}).join(""));
        }
    }
