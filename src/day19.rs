...
impl Instruction {
    fn accept(&self, part: &Part) -> Option<String> {
        match self.filter {
            InstrFilter::Greater(car, val) => {
                if part.get_carac(car) > val {
                    Some(self.out.clone())
                } else {
                    None
                }
            }
            InstrFilter::Lesser(car, val) => {
                if part.get_carac(car) < val {
                    Some(self.out.clone())
                } else {
                    None
                ...
                let instructions = scan
                    .map(|i| {
                        if i.contains(':') {
                            ...
                            if compar.contains('>') {
                                Instruction {
                                    filter: InstrFilter::Greater(carac, val),
                                    out: dest.to_string(),
                                }
                            } else {
                                Instruction {
                                    filter: InstrFilter::Lesser(carac, val),
                                    out: dest.to_string(),
                                }
                            }
                        } else {
                            Instruction {
                                filter: InstrFilter::All,
                                out: i.to_string(),
                            }
                        }
                    })
...
impl Filter {
        ...
        "xmas"
            ...
            .flat_map(|c| {
                ...
                for f in filters.iter().filter(|f| f.target_carac(c)) {
                    match f {
                        InstrFilter::Greater(_, gt) => {
                            max_greater =
                                max_greater.map_or(Some(*gt), |maxgt| Some(max(maxgt, *gt)));
                        }
                        InstrFilter::Lesser(_, lt) => {
                            min_lesser =
                                min_lesser.map_or(Some(*lt), |minlt| Some(min(minlt, *lt)));
                        }
                        InstrFilter::All => {}
                        InstrFilter::Reject => {
                            no_solutions = true;
                ...
                if no_solutions {
                    vec![InstrFilter::Reject]
                } else {
                    [
                        max_greater.filter(|_| !no_solutions).map(|v| InstrFilter::Greater(c, v)),
                        min_lesser.filter(|_| !no_solutions).map(|v| InstrFilter::Lesser(c, v)),
                    ]
                    .into_iter()
                    .flatten()
                    .collect()
    ...
    fn accepted_combinations(&self) -> usize {
        ...
        while !current_paths.is_empty() {
            let new_paths: Vec<(String, Vec<InstrFilter>)> = current_paths
                .iter()
                .flat_map(|(src, path)| {
                ...
                let result = "xmas"
                    ...
                    .map(|c| {
                        ...
                        for f in p.iter().filter(|f| f.target_carac(c)) {
                            match f {
                                InstrFilter::Greater(_, gt) => {
                                    min_greater = min_greater
                                        .map_or(Some(*gt), |mingt| Some(min(mingt, *gt)));
                                }
                                InstrFilter::Lesser(_, lt) => {
                                    max_lesser =
                                        max_lesser.map_or(Some(*lt), |maxls| Some(max(maxls, *lt)));
                                }
                                InstrFilter::All => {
                                    panic!("should not have an All here, since were compacted")
                                }
                                InstrFilter::Reject => {
                                    panic!("Reject should have bee filtered out")
                                }
                            }
                        }
                        max_lesser
                            .unwrap_or(4001)
                            .saturating_sub(1)
                            .saturating_sub(min_greater.unwrap_or(0))
                    })
                    .product::<usize>();
                result
            })
