...
impl FromStr for HeatLossMap {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Self(s.lines().filter(|l|!l.is_empty())
...
fn get_next_moves(heatloss_map: &[Vec<usize>], current:Move, min_len:usize, max_len:usize) -> [Option<Move>;3]{
    let maxx = heatloss_map[0].len();
    let maxy= heatloss_map.len();
    let Move{x,y,dir,l}=current;
    match dir {
        Dir::North => {[if l < max_len && y >0 {Some(Move{x,y:y-1,dir:Dir::North,l:l+1})}else{None},
            if l >= min_len && x >0 {Some(Move{x:x-1,y,dir:Dir::West,l:1})}else{None},
            if l >= min_len && x+1<maxx {Some(Move{x:x+1,y,dir:Dir::East,l:1})}else { None }]}
        Dir::West => {
            [if l < max_len && x >0 {Some(Move{x:x-1,y,dir:Dir::West,l:l+1})}else{None},
                if l >= min_len && y >0 {Some(Move{x,y:y-1,dir:Dir::North,l:1})}else{None},
                if l >= min_len && y+1<maxy {Some(Move{x,y:y+1,dir:Dir::South,l:1})}else { None }]
        }
        Dir::South => {[if l < max_len && y+1 <maxy {Some(Move{x,y:y+1,dir:Dir::South,l:l+1})}else{None},
            if l >= min_len && x >0 {Some(Move{x:x-1,y,dir:Dir::West,l:1})}else{None},
            if l >= min_len && x+1<maxx {Some(Move{x:x+1,y,dir:Dir::East,l:1})}else { None }]}
        Dir::East => {
            [if l < max_len && x+1 < maxx {Some(Move{x:x+1,y,dir:Dir::East,l:l+1})}else{None},
                if l >= min_len && y >0 {Some(Move{x,y:y-1,dir:Dir::North,l:1})}else{None},
                if l >= min_len && y+1<maxy {Some(Move{x,y:y+1,dir:Dir::South,l:1})}else { None }]
...
fn get_minimal_heat_loss(heatloss_map: &[Vec<usize>],min_len: usize, max_len : usize) ->usize{
    ...
    let maxx= heatloss_map[0].len();
    let maxy= heatloss_map.len();
    let end:Move=Move{x:maxx-1,y:maxy-1,dir:Dir::StartStop,l:42};

    let mut previous_moves_to_loss : HashMap<Move,usize>=HashMap::with_capacity(maxx*maxy*4);

    let mut current_moves : Vec<_>= get_next_moves(heatloss_map,Move{x:0,y:0,dir:Dir::StartStop,l:42},min_len,max_len).into_iter().flatten().collect();
    for m in &current_moves {
        previous_moves_to_loss.insert(*m,heatloss_map[m.y][m.x]);
    }
    while !current_moves.is_empty() {
        let mut new_moves=Vec::with_capacity(current_moves.len());
        for current_m in &current_moves {
            let current_loss=previous_moves_to_loss.get(current_m).copied().unwrap();
            for next_m in get_next_moves(heatloss_map, *current_m,min_len,max_len).iter().flatten(){
                let loss=current_loss+heatloss_map[next_m.y][next_m.x];
                if next_m.x==maxx-1 && next_m.y==maxy-1  {
                    if next_m.l < min_len {
                        continue;
                    }
                    previous_moves_to_loss.entry(end).and_modify(|l|*l=min(*l,loss)).or_insert(loss);
                    // goal reached , no need to get next step
                }else{
                    if let Some(l)=previous_moves_to_loss.get(next_m){
                        if *l <= loss {
                            continue
                        }
                    }
                    previous_moves_to_loss.entry(*next_m).and_modify(|l|*l=min(*l,loss)).or_insert(loss);
    ...
    *previous_moves_to_loss.get(&end).unwrap()

}
pub fn carry_lava(){
    let heatloss_map:HeatLossMap = include_str!("../resources/day17_heatloss.txt").parse().unwrap();
