...
fn calibrate(input: &str)-> u32 {
    input.lines().map(|l|{
        let digits: Vec<_> = l.chars().filter(|c|c.is_digit(10)).collect();
        if digits.is_empty() {0} else {
            digits[0].to_digit(10).unwrap_or(0)*10 + digits[digits.len()-1].to_digit(10).unwrap_or(0)
        }
    }).sum()
